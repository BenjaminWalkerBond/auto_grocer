...
class TestCartAddVerification:
    ...
    async def test_returns_error_when_item_not_in_cart_after_add(self):
        ...
        with (
            ...
            patch("auto_grocer_mcp.tools.cart._get_client") as mock_get_client,
        ):
            client = AsyncMock()
...
class TestCartAddWithRetry:
    ...
    async def test_returns_success_on_first_attempt(self):
        ...
        from auto_grocer_mcp.tools.cart import cart_add_with_retry

        with patch("auto_grocer_mcp.tools.cart.cart_add") as mock_cart_add:
