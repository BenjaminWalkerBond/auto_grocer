...
def reset_auth_state():
    ...
    from auto_grocer_mcp.auth.session import _reset_auth_state
    _reset_auth_state()
    yield
    _reset_auth_state()
...
async def test_cart_add_without_confirm_returns_preview():
    ...
    from auto_grocer_mcp.tools.cart import cart_add

    with patch("auto_grocer_mcp.tools.cart.is_authenticated", return_value=True):
...
async def test_cart_add_requires_auth():
    ...
    with patch("auto_grocer_mcp.tools.cart.is_authenticated", return_value=False):
...
def test_cart_check_auth_returns_status():
    ...
    from auto_grocer_mcp.tools.cart import cart_check_auth
...
async def test_cart_get_requires_auth():
    ...
    from auto_grocer_mcp.tools.cart import cart_get
...
async def test_cart_remove_requires_auth():
    ...
    from auto_grocer_mcp.tools.cart import cart_remove
...
async def test_cart_remove_without_confirm_returns_preview():
    ...
    import auto_grocer_mcp.auth.session as session_module
    from auto_grocer_mcp.tools.cart import cart_remove
...
class TestCartAddMany:
    ...
    async def test_requires_auth(self):
        ...
        from auto_grocer_mcp.tools.cart import cart_add_many
    ...
    async def test_success_all_items_added(self):
        ...
        with (
            patch("auto_grocer_mcp.tools.cart.is_authenticated", return_value=True),
            patch("auto_grocer_mcp.tools.cart._get_client", return_value=mock_client),
        ):
            result = await cart_add_many(
                items=[
                    {"product_id": "123", "sku_id": "123-HEB", "quantity": 2},
