# ADR 0002: Self-Hosted Remote MCP Endpoint (host `auto-grocier` from this machine)
...
Today `auto-grocier` ships as a **local stdio server**. `mcp_server.py` ends in
...
- The server holds **sensitive state**: a logged-in HEB session (`auth.json` in the
  `auto_grocier_session` volume), the Claude API key, Gmail IMAP creds, and DB creds. A
  public endpoint with no auth would let anyone spend money and read the account.
- `place_order` is **guarded** (disabled unless `AUTO_GROCIER_ALLOW_PLACE_ORDER=1`) and
...
1. **Transport is selected by env var, stdio stays default.** Introduce
   `AUTO_GROCIER_TRANSPORT` (`stdio` default | `http`). In `main()`:
   ...
   Host/port come from `AUTO_GROCIER_HTTP_HOST` / `AUTO_GROCIER_HTTP_PORT`.
2. **Authentication is mandatory in `http` mode.** The server refuses to start in `http`
   ...
   - A **static bearer token** (`AUTO_GROCIER_HTTP_TOKEN`) checked on every request as the
     minimum bar, and/or
...
4. **Keep `place_order` disabled on the exposed instance.** The remote deployment runs
   with `AUTO_GROCIER_ALLOW_PLACE_ORDER` unset. Paid orders remain a local-only,
...
5. **Run it as a separate, long-lived container** distinct from the on-demand
   ...
   `127.0.0.1`, and shares the same `auto_grocier_session` volume and `.env`. The tunnel
...
- **Phase 0 — Transport switch (local only).**
  - Add `AUTO_GROCIER_TRANSPORT` handling in `mcp_server.main()`; default `stdio`
    unchanged. In `http` mode bind `127.0.0.1:<port>`.
  - Verify: `AUTO_GROCIER_TRANSPORT=http` server answers an MCP handshake on localhost
    (e.g., `curl`/an MCP client), and the default run still works stdio.
- **Phase 1 — Auth gate.**
  - Require `AUTO_GROCIER_HTTP_TOKEN` in `http` mode; reject unauthenticated/incorrect
...
- **Phase 2 — Containerized HTTP service.**
  - Add an `mcp-http` compose service (detached, `127.0.0.1`-only publish,
    `restart: unless-stopped`, shared session volume, `AUTO_GROCIER_ALLOW_PLACE_ORDER`
