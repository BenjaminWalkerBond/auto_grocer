...
description: 'Refresh HEB GraphQL persisted-query hashes for the auto-grocer MCP server. Use when MCP tools return OPERATION_NOT_CAPTURED, persisted-query/hash mismatch errors, or product search / add-to-cart / checkout calls fail after an HEB site update. Captures fresh hashes INSIDE the Docker container (Xvfb, no host browser), which writes persisted_queries.json directly into the session volume, then calls refresh_session.'
...
`auto_grocer_session` volume that the MCP server reads. This means **no browser
...
2. Reload the hashes/session in the server: call `mcp_auto-grocer_refresh_session`.
3. Verify: call `mcp_auto-grocer_search_products` (e.g. "milk") — expect results
   with real `product_id`s, not `OPERATION_NOT_CAPTURED` or suggestion-only rows.

## Notes
- The capture runs the `auto_grocer.session_maintenance.run` module (MODE=nodriver
...
- The container writes `auth.json`, `persisted_queries.json`, and
  `captured_operations.json` straight into the `auto_grocer_session` volume — no
