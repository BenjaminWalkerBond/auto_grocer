...
the `auto_grocer_session` volume. This means **no browser window opens on your
...
1. Confirm the session is stale: call `mcp_auto-grocer_auth_status`. If already
...
3. Reload the session in the server: call `mcp_auto-grocer_refresh_session`.
4. Verify with `mcp_auto-grocer_auth_status` → expect `{"authenticated": true}`.

## Notes
- The login runs the `auto_grocer.session_maintenance.run` module (MODE=nodriver
  OPERATION=login_export). The old `grocery_browser.run` module no longer exists.
- Session file lands in the `auto_grocer_session` volume
